...
```
Now, we are ready to move the robot toward the object to grasp it.  `pybullet.setJointMotorControl2([objectUid],[jointIndex],[controller],[targetPosition])` will control the robot joints. The joint index is based on what it is defined in the decription file (e.g. URDF.) The following positions the robot at the top of the object:
```
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA0MTgxOTQ5MywtMTcyOTIwNDMwNV19
-->